...
async function createGroupVenue(req, res, next) {
  ...
  try {
    ...
    req.app.io.emit('membership', { msg: `A new venue is added to "${group.name}" group!`, userIds: memberIds });
    res.json({
      id: newVenue.id,
...
async function editVenue(req, res, next) {
  ...
  req.app.io.emit('membership', { msg: `A venue of "${group.name}" group is updated!`, userIds: memberIds });
  res.json({
    id: updatedVenue.id,
...
async function deleteVenue(req, res, next) {
  ...
  req.app.io.emit('membership', { msg: `A venue of "${group.name}" group is deleted!`, userIds: memberIds });
