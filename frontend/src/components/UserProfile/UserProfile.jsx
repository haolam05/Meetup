...
function UserProfile() {
  ...
  const [ownedGroupsRef, joinedGroupsRef, hostedEventsRef, attendedEventsRef] = [useRef(null), useRef(null), useRef(null), useRef(null)];
        ...
        <div className="group details">
          ...
          <div id="user-wrapper-container">
            ...
            <div id="user-info-wrapper">
              <div id="user-info">
                ...
                <div id="user-titles">
                  <div>Username</div>
                  <div>Email</div>
                  <div className="clickable" onClick={() => ownedGroupsRef.current.scrollIntoView()}>Owned</div>
                  <div className="clickable" onClick={() => joinedGroupsRef.current.scrollIntoView()}>Joined</div>
                  <div className="clickable" onClick={() => hostedEventsRef.current.scrollIntoView()}>Hosted</div>
                  <div className="clickable" onClick={() => attendedEventsRef.current.scrollIntoView()}>Attend</div>
                </div>
                <div id="user-data">
            ...
            <div id="user-btns">
              ...
              <OpenModalButton
                buttonText="Delete"
                modalComponent={<DeleteUser userId={user.id} title="Account" />}
              />
            </div>
          </div>
        </div>
        <div id="user-details">
          {ownedGroups.length ? <h2 className="subheading" ref={ownedGroupsRef}>Groups Owned ({ownedGroups.length})</h2> : ''}
          {ownedGroups.length ? ownedGroups.map(group => <ManageGroup key={group.id} group={group} user={user} />) : ''}

          {joinedGroups.length ? <h2 className="subheading" ref={joinedGroupsRef}>Groups Joined ({joinedGroups.length})</h2> : ''}
          {joinedGroups.length ? joinedGroups.map(group => <ManageGroup key={group.id} group={group} user={user} />) : ''}

          {hostedEvents.length ? <h2 className="subheading" ref={hostedEventsRef}>Events Hosted ({hostedEvents.length})</h2> : ''}
          ...
          {attendedEvents.length ? <h2 className="subheading" ref={attendedEventsRef}>Events Attended ({attendedEvents.length})</h2> : ''}
